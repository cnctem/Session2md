...
- 删除单个会话，或多选会话，也可以按供应商或项目目录整组选择删除。

## 数据与安全

Session2md 直接读取各工具的本地会话存储，不连接 CC Switch，不启动代理，不上传会话内容，也不会启动或恢复 CLI 会话。界面中的恢复命令仅在用户主动操作时复制到剪贴板。

删除是需要确认且不可逆的操作。确认后，Session2md 会通过对应供应商的清理逻辑修改本地会话文件或数据库；应用无法恢复已删除的会话，请在删除前备份重要数据。应用自身的语言、主题和目录覆盖设置会单独保存。导出功能只会写入用户在系统保存对话框中选择的 Markdown 文件。
